...
export default function AddEmployee({ showToast }) {
  const [form, setForm] = useState({ Employee_Name: '', CNIC: '', Phone_Number: '', Salary: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else { showToast(`Employee "${form.Employee_Name}" added!`); setForm({ Employee_Name: '', CNIC: '', Phone_Number: '', Salary: '' }); loadEmployees(); }
      ...
      <div className="form-container mb-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-grid">
            ...
            <div className="form-group"><label>Fixed Monthly Salary (PKR)</label><input type="number" min="0" placeholder="e.g. 20000" value={form.Salary} onChange={e => setForm({...form, Salary: e.target.value})} /></div>
          </div>
      ...
      <div className="table-container">
        ...
        {employees.length === 0 ? <div className="empty-state"><div className="icon"><UsersIcon size={36}/></div><h3>No Employees</h3><p>Add employees to assign them to sales.</p></div> : (
          <table className="data-table">
            <thead><tr><th>Name</th><th>CNIC</th><th>Phone</th><th>Salary</th><th>Date Added</th><th>Status</th><th>Action</th></tr></thead>
            <tbody>{employees.map((e,i) => (
              <tr key={i}><td style={{fontWeight:600}}>{e.Employee_Name}</td><td>{e.CNIC||'-'}</td><td>{e.Phone_Number||e.Phone||'-'}</td><td>PKR {Number(e.Salary || 0).toLocaleString()}</td><td>{e.Date_Added}</td><td><span className={`status-badge ${e.Status==='Active'?'status-active':'status-overdue'}`}>{e.Status}</span></td><td>{e.Status==='Active' && <button className="btn btn-danger btn-sm" onClick={() => handleDelete(e.Employee_ID)}>Deactivate</button>}</td></tr>
            ))}</tbody>
          </table>
        )}
      </div>
