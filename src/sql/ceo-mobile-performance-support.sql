...
create index if not exists idx_appeals_town_status_created_at
  on public.appeals (town_name, status, created_at desc);

create index if not exists idx_daily_entries_review_created_at
  on public.daily_entries (review_status, created_at desc);

create index if not exists idx_daily_entries_date_town_type
  on public.daily_entries (date, town_name, type);

create index if not exists idx_notifications_dismissed_created_date
  on public.notifications (dismissed, created_date desc);

create index if not exists idx_media_daily_receipts
  on public.media_library (type, report_date, created_at desc);

create index if not exists idx_all_sales_town_created_at
  on public.all_sales (town_name, created_at desc);

create index if not exists idx_users_role_town_online
...
as $$
  select
    ...
    a.town_name::text,
    case
      when jsonb_typeof(a.requested_data::jsonb) = 'object' then a.requested_data::jsonb
      else '{}'::jsonb
